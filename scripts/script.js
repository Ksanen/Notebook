class DataManager {
  ...
  static checkIfDataAreValid(data) {
    ...
    if (JSON.stringify(keys) !== JSON.stringify(validKeys)) {
      return false;
    }
    ...
    if (JSON.stringify(optionsKeys) !== JSON.stringify(validOptionsKeys)) {
      return false;
    }
    if (
      typeof data.options.darkMode !== "boolean" &&
      typeof data.options.darkMode !== "undefined"
    ) {
      return false;
    }
    if (typeof data.options.moveNoteToTheBin !== "boolean") {
      return false;
    }
  ...
  static checkIfNoteIsValid(note) {
    ...
    for (let i = 0; i < lastEditedValues.length; i++) {
      if (!Number.isInteger(parseInt(lastEditedValues[i]))) {
        return false;
      }
    }
    return true;
...
class UIManager {
  ...
  closeSearchBar() {
    requestAnimationFrame(() => {
      ...
      document
        .querySelector(".search-container_input")
        .classList.remove("search-container_input--open");
      document
        .querySelector(".search-container_icon")
        .classList.remove("search-container_icon--end");
    });
  ...
  openSearchBar(searchIcon) {
    ...
    setTimeout(() => {
      document
        .querySelector(".search-container_input")
        .classList.add("search-container_input--open");
  ...
  loadLanguagesInSelect() {
    ...
    const languages = this.notebook.languageData;
  ...
  loadNotes(notes, page) {
    ...
    notes.forEach((note) => {
      ...
      let withoutTitle = this.notebook.languageData
        ? this.notebook.MESSAGES.text.withoutTitle
        : "bez tytułu";
      let title = note.title === "" ? `${withoutTitle}` : note.title;
      let lastSave = this.notebook.languageData
        ? this.notebook.MESSAGES.text.lastSave
        : "Ostatni zapis";
  ...
  removeCategoriesOnNav() {
    ...
    categoriesOnNav = categoriesOnNav.filter(
      (category) => category.getAttribute("category-name") !== "all"
    );
    ...
    switch (fragment) {
      ...
      case "notes":
        ...
        if (this.notebook.currentCategory === 1) {
          ...
          let notesToLoad = this.notebook.data.notes.filter(
            (note) => note.category === this.notebook.currentCategory
          );
...
class EventManager {
  ...
  addDefaultEventListeners() {
    document
      ...
      .addEventListener("click", this.notebook.UIManager.hideOptions);
    let iconOpeningOptions = Array.from(
      document.getElementsByClassName("icon-container--open-options")
    );
    ...
    document.querySelector(".btn-save").addEventListener("click", () => {
      this.notebook.UIManager.showPopUpInformation(
        this.notebook.MESSAGES.announcements.noteIsSaved
      );
    });
      ...
      fileReader.onload = () => {
        try {
          ...
          if (DataManager.checkIfDataAreValid(data)) {
            ...
            this.notebook.UIManager.showPopUpInformation(
              this.notebook.MESSAGES.announcements.dataLoaded
            );
            DataManager.saveApplicationData(this.data);
            ...
            this.notebook.UIManager.showPopUpInformation(
              this.notebook.MESSAGES.announcements.unableToLoad
            );
          }
        } catch (e) {
          this.notebook.UIManager.showPopUpInformation(
            this.notebook.MESSAGES.announcements.parsingError
          ); //
  ...
  addEventListenersToSearchBar() {
    ...
    searchContainer.addEventListener("click", () =>
      this.notebook.UIManager.openSearchBar(searchContainer)
    );
    ...
    searchContainerInput.addEventListener("input", () =>
      this.searchBarHandler(searchContainerInput)
    );
  ...
  searchBarHandler(searchContainerInput) {
    ...
    notes =
      this.notebook.currentCategory === 1
        ? this.data.notes
        : this.data.notes.filter((note) => note.category === this.notebook.currentCategory);
    ...
    notes = notes.filter((note) => {
      ...
      const phrase = this.notebook.languageData
        ? this.notebook.MESSAGES.text.withoutTitle.toLowerCase()
        : "bez tytułu";
      if (note.title === "") {
  ...
  optionEventListener = (option) => {
    ...
    switch (option.getAttribute("option")) {
      case "restoreDeletedNotes":
        ...
        this.notebook.UIManager.showPopUpInformation(
          this.notebook.MESSAGES.announcements.restoredAllNotes
        );
        DataManager.saveApplicationData(this.data);
        this.notebook.UIManager.reloadFragmentOfThePage("deletedNotes");
      ...
      case "emptyTheBin":
        ...
        this.notebook.UIManager.showPopUpInformation(
          this.notebook.MESSAGES.announcements.BinEmptied
        );
        DataManager.saveApplicationData(this.data);
        this.notebook.UIManager.reloadFragmentOfThePage("deletedNotes");
        this.notebook.UIManager.hideOptions();
      ...
      case "deleteNote":
        ...
        this.notebook.UIManager.showPopUpInformation(
          this.notebook.MESSAGES.announcements.NoteDeleted
        );
        this.notebook.UIManager.reloadFragmentOfThePage("notes");
        this.notebook.UIManager.hideOptions();
        break;
        ...
        if (document.querySelector(`input[name="noteOption"]:checked`)) {
          ...
          switch (noteOption.value) {
            case "restore":
              ...
              DataManager.saveApplicationData(this.data);
              this.notebook.UIManager.showPopUpInformation(
                this.notebook.MESSAGES.announcements.noteRestored
              );
              break;
            case "delete":
      ...
      case "deleteCategoryFromNote":
        const note = this.notebook.data.notes.find(
          (note) => note.id === this.notebook.data.currentNoteId
        );
        note.category = "";
        this.notebook.UIManager.showPopUpInformation(
          this.notebook.MESSAGES.announcements.categoryDeletedFromNote
        );
        this.notebook.UIManager.hideOptions();
        DataManager.saveApplicationData(this.data);
        break;
...
class Notebook {
  ...
  async loadPage() {
    ...
    this.MESSAGES = this.languageData[`${this.currentLanguage}`];
  ...
  changeLanguage(lang) {
    ...
    if (!Object.keys(this.languageData).find((language) => language === lang)) {
      return;
    }
    this.data.options.lang = lang;
    this.MESSAGES = this.languageData[`${this.data.options.lang}`];
    this.UIManager.loadLanguageOnPage();
    DataManager.saveApplicationData(this.data);
  }
  renameCategory() {
  ...
  sortNotesByDate() {
    this.data.notes = this.data.notes.sort((a, b) => {
      ...
      b = b.lastEdited;
      return (
        new Date(`${b.year}-${b.month}-${b.day}T${b.hour}:${b.minute}:${b.second}`).getTime() -
        new Date(`${a.year}-${a.month}-${a.day}T${a.hour}:${a.minute}:${a.second}`).getTime()
      );
    });
